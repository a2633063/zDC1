...
uart_config(uint8 uart_no) {
	...
	if (uart_no == UART0) {
		//set rx fifo trigger
		WRITE_PERI_REG(UART_CONF1(uart_no), ((100 & UART_RXFIFO_FULL_THRHD) << UART_RXFIFO_FULL_THRHD_S) |
#if UART_HW_RTS
...
LOCAL uint8_t redat_num = 0;
LOCAL uint8_t head = 0;
LOCAL uint32_t Uk = 0, Ut = 0, Ik = 0, It = 0, Pk = 0, Pt = 0;
LOCAL uint8_t Adj = 0;
LOCAL uint16_t CF = 0;
LOCAL uint8_t sum = 0;
...
//				if (Adj & 0x10)
				...
				if (power_num > TEMP_COUNT) {	//至少获取5组数据后再开始确认功率电压电流数据
					...
					for (i = 0; i < TEMP_COUNT; i++) {
						//计算和
						voltage_sum += voltage_temp[i];
						power_sum += power_temp[i];
						current_sum += current_temp[i];

						//计算最大值
						if (voltage_max < voltage_temp[i])
							voltage_max = voltage_temp[i];
						if (power_max < power_temp[i])
							power_max = power_temp[i];
						if (current_max < current_temp[i])
							current_max = current_temp[i];

						//计算最小值
						if (voltage_min > voltage_temp[i])
							voltage_min = voltage_temp[i];
						if (power_min > power_temp[i])
							power_min = power_temp[i];
						if (current_min > current_temp[i])
							current_min = current_temp[i];
					}
